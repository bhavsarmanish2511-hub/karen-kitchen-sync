    ...
    <Card className={cn(
      ...
      <div className="p-4">
        ...
        {affectedProducts && affectedProducts.length > 0 && (
          <div className="space-y-2 mb-3 p-3 bg-muted/30 rounded-md border border-border/40">
            <div className="text-xs font-semibold text-foreground mb-2">
              Affected Products (Highest Impact):
            </div>
            {affectedProducts.slice(0, 2).map((product, idx) => (
              <div key={idx} className="space-y-1 pb-2 border-b border-border/40 last:border-0 last:pb-0">
                <div className="flex items-start justify-between">
                  <div className="font-medium text-sm text-foreground">{product.name}</div>
                  {product.impactPercentage && (
                ...
                <div className="text-xs text-muted-foreground">
                  <span className="font-medium">SKUs:</span> {product.skus.slice(0, 2).join(", ")}
                  {product.skus.length > 2 && ` +${product.skus.length - 2} more`}
                </div>
                {product.routes && product.routes.length > 0 && (
                  <div className="text-xs text-muted-foreground">
                    <span className="font-medium">Route:</span> {product.routes[0]}
                  </div>
                )}
              </div>
            ))}
            {affectedProducts.length > 2 && (
              <div className="text-xs text-muted-foreground pt-1">
                +{affectedProducts.length - 2} more product{affectedProducts.length - 2 > 1 ? 's' : ''} affected
              </div>
            )}
          </div>
        )}
