...
interface SupplyChainMapMainProps {
  zoomToLocation?: {
    coordinates: [number, number];
    name: string;
  } | null;
...
export const SupplyChainMapMain = ({ zoomToLocation }: SupplyChainMapMainProps) => {
  ...
  // Handle zoom to location with alert marker
  useEffect(() => {
    if (!map.current || !zoomToLocation) return;

    // Remove previous alert marker if exists
    if (alertMarkerRef.current) {
      alertMarkerRef.current.remove();
    }

    // Create pulsing red marker for alert location
    const el = document.createElement('div');
    ...
    `;
    if (!document.getElementById('alert-marker-style')) {
      style.id = 'alert-marker-style';
      document.head.appendChild(style);
    }

    const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(
      `<div class="p-2">
        <h3 class="font-bold text-sm text-red-600">Alert Location</h3>
        <p class="text-xs">${zoomToLocation.name}</p>
      </div>`
    );

    alertMarkerRef.current = new mapboxgl.Marker(el)
      .setLngLat(zoomToLocation.coordinates)
      .setPopup(popup)
      .addTo(map.current);

    // Zoom to the location with animation
    map.current.flyTo({
      center: zoomToLocation.coordinates,
      zoom: 8,
      pitch: 60,
      duration: 2500,
      essential: true
    ...
    setTimeout(() => {
      alertMarkerRef.current?.togglePopup();
    }, 2500);
  }, [zoomToLocation]);
